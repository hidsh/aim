...
<body>
  <form name="opt" method="post">
    <input type="hidden"        name="q_1" value="none" />
    ...
    <hr/>
    <input type="hidden"     name="q_2" value="none" />
    ...
    <hr/>
    <input type="hidden"        name="q_3" value="none" />
   ...
   def receive(self, **ans_dict):
      ...
      if ans_dict:
         ...
         for x in answers:
             ...
             s += '  <div>問%d の答え：%s</div>' % (x.qnum, reduce(concat, x.ans))
